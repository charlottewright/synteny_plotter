
# args for running interactively in R
args <- list()
args$alg_file <- '../sup_tables/TableS4_Merian_element_definitions.tsv'
args$busco1 <- 'test_data/Melitaea_cinxia.tsv'
#args$busco2 <-'test_data/Vanessa_cardui.tsv'
#args$busco3 <- 'test_data/Inachis_io.tsv'
args$chrom1 <- 'test_data/Melitaea_cinxia_info.tsv'
#args$chrom2 <- 'test_data/Vanessa_cardui_info.tsv'
#args$chrom3 <- 'test_data/Inachis_io_info.tsv'
args$alpha <- 0
args$output_prefix <- 'test_three_species'
args$busco_list <- c('test_data/Vanessa_cardui.tsv', 'test_data/Inachis_io.tsv')
args$chrom_list <- c('test_data/Vanessa_cardui_info.tsv','test_data/Inachis_io_info.tsv')
args$filter <- 5

args <- list()
args$alg_file <- '../sup_tables/TableS4_Merian_element_definitions.tsv'
args$busco1 <- 'marcela_data/Sepiola_atlantica.tsv'
#args$busco2 <-'test_data/Vanessa_cardui.tsv'
#args$busco3 <- 'test_data/Inachis_io.tsv'
args$chrom1 <- 'marcela_data/Sepiola_atlantica_info.tsv'
#args$chrom2 <- 'test_data/Vanessa_cardui_info.tsv'
#args$chrom3 <- 'test_data/Inachis_io_info.tsv'
args$alpha <- 0
args$output_prefix <- 'test_marcela'
args$busco_list <- c('marcela_data/Siphonodentalium_dalli.tsv')
args$chrom_list <- c('marcela_data/Siphonodentalium_dalli_info.tsv')
args$filter <- 37


args <- list()
args$alg_file <- '../sup_tables/TableS4_Merian_element_definitions.tsv'
args$busco1 <- 'marcela_data/xbHipHipp8_full_table.tsv'
args$chrom1 <- 'marcela_data/xbHipHipp8.test.auto_generated.tsv'
args$alpha <- 0
args$output_prefix <- 'test_marcela_hiphip'
args$busco_list <- c('marcela_data/xbTriDera4_full_table.tsv', 'marcela_data/Tridacna_crocea_full_table.tsv')
args$chrom_list <-c('marcela_data/xbTriDera4.test.auto_generated.tsv', 'marcela_data/Tridacna_crocea.test.hiphip.auto_generated.tsv')
args$filter <- 37
